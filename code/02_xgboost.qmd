...
# 1. Collecting predictions from final fit
preds <- final_fit %>%
  collect_predictions()

# 2. Computing RMSE & R² using yardstick
metrics_tbl <- preds %>%
...
# 3. Plotting Predicted vs Observed and annotate with metrics
ggplot(preds, aes(x = yield, y = .pred)) +
  ...
  annotate(
    "text",
    x     = min(preds$yield, na.rm = TRUE),
    y     = max(preds$.pred, na.rm = TRUE),
